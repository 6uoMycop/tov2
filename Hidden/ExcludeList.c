...
NTSTATUS InitializeExcludeListContext(PExcludeContext Context, UINT32 Type)
{
	...
	{
		_InfoPrint("Error, invalid exclude list type: %d", Type);
	...
	if (!cntx)
	{
		_InfoPrint("Error, can't allocate memory for context: %p", Context);
...
NTSTATUS AddExcludeListEntry(ExcludeContext Context, PUNICODE_STRING FilePath, UINT32 Type, PExcludeEntryId EntryId, ExcludeEntryId ParentId)
{
	...
	if (cntx->type != Type)
	{
		_InfoPrint("Warning, type isn't equal: %d != %d", cntx->type, Type);
		return STATUS_INVALID_MEMBER;
	}

	if (FilePath->Length == 0 || FilePath->Length >= MAX_PATH_SIZE)
	{
		_InfoPrint("Warning, invalid string size : %d", (UINT32)FilePath->Length);
		return STATUS_ACCESS_DENIED;
	}
	...
	if (entry == NULL)
	{
		_InfoPrint("Warning, exclude file list is not NULL : %p", cntx);
		return STATUS_ACCESS_DENIED;
	}
