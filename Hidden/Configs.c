...
NTSTATUS InitializeConfigs(PUNICODE_STRING RegistryPath)
{
	...
	{
		_InfoPrint("Error, can't open config registry key");
		return status;
	}
	...
	if (!g_configContext)
	{
		_InfoPrint("Error, can't allocate memory for the config context");
	...
	_InfoPrint("Config is initialized");
...
NTSTATUS DestroyConfigs()
{
	...
	_InfoPrint("Config is destroyed");
	return STATUS_SUCCESS;
}
...
NTSTATUS CfgEnumConfigsTable(enum CfgMultiStringTables Table, CfgMultiStringCallback Callback, PVOID Params)
{
	...
	while (length > 1)
	{
		...
		length -= ((size_t)inx + (size_t)delta) * sizeof(WCHAR);
		buffer += ((size_t)inx + (size_t)delta);
